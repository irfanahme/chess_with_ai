...
try:
    ...
    from env.posible_moves import possible_moves, possible_move_index, move_to_index
except:
    from replay_buffer import ReplayBuffer
    from posible_moves import possible_moves, possible_move_index, move_to_index
...
class DeepQNetwork(object):
  ...
  def __init__(self, session,
    ...
    self.var_lists = tf.get_collection(tf.GraphKeys.VARIABLES)
    self.session.run(tf.initialize_variables(self.var_lists))
  ...
  def create_variables(self):
    ...
    with tf.name_scope("predict_actions"):
      # raw state representation  
          self.states = tf.placeholder(tf.float32, (None,s[0], s[1], s[2]), name="states")
#          self.v_value =  tf.placeholder(tf.float32, (None,), name="v_value")
          # initialize Q network
          with tf.variable_scope("q_network", reuse=False):
               self.q_output_p, self.q_output_v = self.q_network(self.states, myscope = "q_network",residual_tower = self.block)
          # predict actions from Q network
          self.action_scores = tf.identity(self.q_output_p, name="action_scores")
          self.v_value = tf.identity(self.q_output_v, name="v_value")

          tf.summary.histogram("action_scores", self.action_scores)
          tf.summary.histogram("v_value", self.v_value)

          self.predicted_actions = self.action_scores
          
    # estimate rewards using the next state: r(s_t,a_t) + argmax_a Q(s_{t+1}, a)
    with tf.name_scope("estimate_future_rewards"):
      self.next_states = tf.placeholder(tf.float32, (None,s[0], s[1], s[2]), name="next_states")
      ...
      if self.double_q_learning:
            ...
            with tf.variable_scope("q_network", reuse=tf.AUTO_REUSE):
              self.q_next_output_p, self.q_next_output_v = self.q_network(self.next_states , myscope = "q_network",residual_tower = self.block)
            self.action_selection = tf.argmax(tf.stop_gradient(self.q_next_output_p), 1, name="action_selection")
            tf.summary.histogram("action_selection", self.action_selection)
            tf.summary.histogram("v_value_selection", self.q_next_output_v )
            ...
            with tf.variable_scope("target_network"):
              self.target_output_p, self.target_output_v = self.q_network(self.next_states, myscope = "target_network", residual_tower = self.block)
              self.target_output_p = self.target_output_p * tf.cast(self.action_selection_mask, tf.float32)
            self.action_evaluation = tf.reduce_sum(self.target_output_p, reduction_indices=[1,])
            tf.summary.histogram("action_evaluation", self.action_evaluation)
            tf.summary.histogram("value_evaluation", self.target_output_v)

            self.target_values = self.action_evaluation * self.next_state_mask
      else:
        # initialize target network
            with tf.variable_scope("target_network"):
              self.target_outputs = self.q_network(self.next_states, myscope = "target_network", residual_tower = self.block)
  ...
  def eGreedyAction(self, states,available_moves, explore=True):
     if explore and self.exploration > random.random():
         random.shuffle(available_moves)
         return available_moves[0]
     
     else:     
        prob_distribution = self.session.run(self.predicted_actions, {self.states: states})[0]
        avaible_moves_index = possible_move_index(available_moves)
        x = []
        for i in avaible_moves_index:
            x.append(prob_distribution[i])
        m = max(x)
        moves = possible_moves
        highest_Problitity_index = [i for i, j in enumerate(x) if j == m]
        move = moves[avaible_moves_index[highest_Problitity_index[0]]]
        return move

  def get_p_V(self,  states):
        p_values, v_values =  self.session.run([self.predicted_actions,self.v_value], {self.states: states})
        return p_values, v_values[0]
  ...
  def updateModel(self):
    # not enough experiences yet
        ...
        states          = np.zeros((self.batch_size, self.state_dim[0], self.state_dim[1], self.state_dim[2]))
        rewards         = np.zeros((self.batch_size,))
        action_mask     = np.zeros((self.batch_size, self.num_actions))    
        next_states     = np.zeros((self.batch_size, self.state_dim[0], self.state_dim[1], self.state_dim[2]))

...
class ChessEnv:
    ...
    def get_current_player(self):
        if self.bot_player == chess.BLACK:    #"white player is winner")
                 return 1
        elif self.bot_player == chess.WHITE:   #("black player is winner")
                return 1
    def game_result(self):
        if self.winner is None:
            self.result = self.board.result(claim_draw = True)
            if self.result == '1-0':    #"white player is winner")
                 return 'White'
            elif self.result == '0-1':   #("black player is winner")
                return 'Black'
            else:   # print("MAtch is draw")
                return 'draw'
